...
export default function PokerTable() {
  ...
  const checkAllDataLoaded = () => {
    console.log(`📊 Checking data: table=${tableDataLoaded}, socket=${socketConnected}`);
    if (tableDataLoaded && socketConnected) {
      console.log('✅ Table ready to render!');
      setLoading(false);
      setError(null);
    }
  };
  ...
  const isWaiting = !gameState || gameState.status === 'waiting' || seatedPlayers < 2;
  ...
  console.log(`   Game status: ${gameState?.status || 'no game'}`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-900 to-green-950 p-4">
      ...
      <div className="relative bg-gradient-to-b from-green-800 to-green-900 rounded-3xl p-8 border-4 border-amber-800/50 shadow-2xl">
        ...
        {!needsMorePlayers && !isWaiting && gameState && isMyTurn && (
