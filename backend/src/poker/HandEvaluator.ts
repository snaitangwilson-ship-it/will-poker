...
export class HandEvaluator {
  ...
  private static evaluateFive(cards: Card[]): HandResult {
    ...
    const valuesByCount: { [count: number]: number[] } = {};
    for (const [rankStr, count] of Object.entries(rankCounts)) {
      const v = this.getRankValue(rankStr as any);
  ...
  public static compareHands(a: HandResult, b: HandResult): number {
  ...
  private static getCardValue(card: Card): number {
    const rank = card.slice(0, -1) as any;
    return Deck.getRankValue(rank);
  }

  private static getCardSuit(card: Card): string {
    return card.slice(-1);
  }

  private static getRankValue(rank: string): number {
    return Deck.getRankValue(rank as any);
  }
