import { CARD_RANKS, HAND_RANK_ORDER } from '../../config/constants';

export class HandEvaluator {
  static evaluate(hand: string[], community: string[]): {
    rank: string;
    rankValue: number;
    kickers: number[];
    cards: string[];
  ...
  private static getRankCounts(cards: string[]): Map<string, number> {
  ...
  private static isFlush(cards: string[]): boolean {
  ...
  private static getStraightCards(cards: string[]): string[] {
  ...
  private static getFlushCards(cards: string[]): string[] {
  ...
  static compareHands(hand1: any, hand2: any): number {
