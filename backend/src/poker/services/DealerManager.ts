export class DealerManager {
  rotateDealer(players: any[]): number {
    const seated = players.filter(p => p.isActive && !p.isSitOut);
    if (seated.length === 0) throw new Error('No active players');
